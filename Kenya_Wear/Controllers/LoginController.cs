...
using System.Text;
using static Kenya_Wear.Helpers.CryptoHelper;
using static Kenya_Wear.Helpers.HttpClientHelper;
...
namespace Kenya_Wear.Controllers
{
    public class LoginController : BaseController
    {
        ...
        public LoginController(ILoggerManager logger,IWebHostEnvironment environment, DBHandler mydbhandler)
        {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (Cryptographer.Encrypt(entity.password).Replace("=", "") == dt.Rows[0]["password"].ToString())
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // Set a persistent cookie (expires after a specified time)
                                    var authProperties = new AuthenticationProperties
                                    {
                                        IsPersistent = true,
                                        ExpiresUtc = DateTime.UtcNow.AddMonths(1) // Adjust the expiration time as needed
                                    };
                                    
                                    // Sign in the user with the session cookie
                                    SignInRemember(dt.Rows[0]["email"].ToString(), authProperties);
        ...
        // Update the SignInRemember method to accept AuthenticationProperties
        // Update the SignInRemember method to accept AuthenticationProperties
        private void SignInRemember(string userName, AuthenticationProperties authProperties)
        {
            // Clear any lingering authentication data
            // FormsAuthentication.SignOut();

            // Write the authentication cookie with the provided AuthenticationProperties
            // FormsAuthentication.SignOut();
        ...
        // Add more claims as needed
            ...
            // Use SignInAsync with the provided AuthenticationProperties
